...
const quiz_take = {
    ...
    addQuizInstance(info, callback) {
        return db.query(
            ...
            [info.user_id, info.quiz_id, 0, info.date],
            callback
        );
    },
    ...
    addAnswerGiven(info, callback) {
        return db.query(
            "insert into answer_given (question_id, answer_id, quiz_instance) values(?,?,?)",
            [info.question_id, info.answer_id, info.quiz_instance],
            callback
        );
    }
};
